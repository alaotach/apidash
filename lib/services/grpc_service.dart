...
class GrpcService {
  ...
  /// Loads service descriptors from uploaded .proto files via local protoc.
  Future<GrpcRequestModel> discoverFromProtoFiles(GrpcRequestModel model) async {
    final endpoint = '${model.host}:${model.port}';
    if (model.protoFiles.isEmpty) {
      return model.copyWith(
        descriptorSource: GrpcDescriptorSource.protoUploadRequired,
        availableServices: const [],
        methodsByService: const {},
        statusCode: 'INVALID_ARGUMENT',
        statusMessage: 'No .proto files selected.',
        errorMessage: 'Upload one or more .proto files to continue.',
      );
    }
    ...
    try {
      ...
      return model.copyWith(
        ...
        methodsByService: discovered.$2,
        statusCode: 'OK',
        statusMessage: 'Proto files loaded successfully.',
        errorMessage: null,
      );
    } catch (e) {
      return model.copyWith(
        ...
        methodsByService: const {},
        statusCode: 'UNAVAILABLE',
        statusMessage: 'Failed to load uploaded .proto files.',
        errorMessage: e.toString(),
      );
    }
  }
  ...
  Future<Map<String, $descriptor.FileDescriptorProto>> _loadDescriptorsFromProtoFiles(
    List<String> protoFiles,
  ) async {
    final existingFiles = protoFiles
        .map((path) => File(path))
    ...
    final args = <String>[
      '--include_imports',
      '--descriptor_set_out=${outFile.path}',
    ...
    if (!outFile.existsSync()) {
      throw _GrpcServiceException('Descriptor set was not generated by protoc.');
    }
    ...
    try {
      ...
      // Best-effort cleanup for temporary descriptor file.
    }

    final fileDescriptorSet = $descriptor.FileDescriptorSet.fromBuffer(bytes);
    final descriptorMap = <String, $descriptor.FileDescriptorProto>{};
    for (final fileDesc in fileDescriptorSet.file) {
      descriptorMap[fileDesc.name] = fileDesc;
    }
    return descriptorMap;
  }
